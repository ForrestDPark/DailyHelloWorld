...
- **출처 링크 의무화(★ 2026-08-09)**: 기업 홈페이지가 확인되면 `1. 기업 개황` 바로 아래에 홈페이지·DART 공시검색·DART 기업개황 링크를 코드가 직접 삽입한다. 뉴스 입력도 제목만 넘기지 않고 `[기사 제목](원문 URL)` 형태로 AI에 전달하며, 내부거래·승계·계열분리·소송·과로 등 기사에서 가져온 판단은 해당 문장 바로 뒤에 기사 링크를 붙이도록 강제한다. 링크가 없는 구체적 사실은 쓰지 않고 `정보 부족`으로 남긴다. 맨 아래 `참고 링크`에는 사용한 DART·뉴스 원문을 다시 모아 전체 출처를 훑을 수 있게 한다.
