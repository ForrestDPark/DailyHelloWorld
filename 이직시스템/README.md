...
**★ 2026-08-09 최상위 "🎴 이직시스템" 추천 인덱스** — 맨 위의 `📋 최근 추천 기록` 토글(`TOP_INDEX_TOGGLE_ID`)에 공고·경진대회 링크를 최신순 한 줄 목록으로 쌓는다. 상세 분석 전문은 각 하위 페이지에만 두어 최상위 페이지를 가볍게 유지한다. 로컬 `data/top_index_history.json`을 원본으로 삼고, 이 워크스페이스에서 지원되지 않는 Notion `after` 파라미터는 쓰지 않는다. 순서를 바꿀 때는 토글의 기존 children을 삭제한 뒤 전체 목록을 원하는 순서로 다시 작성한다.
