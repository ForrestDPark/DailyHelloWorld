...
### 전체 공고 웹앱 대시보드 (2026-09-10)

- `/career/`에서 Notion을 열지 않고 SQLite에 누적된 전체 공고를 직접 확인한다.
- 오늘 새 공고·오늘 확인한 공고·전체 보관 수·마지막 수집 시각을 첫 화면에 표시한다.
- 회사·직무·기술·지역 검색, 출처 필터, 최근 확인·신규·적합도·마감순 정렬과
  40개 단위 더 보기를 지원한다.
- 공고 카드를 누르면 지역·경력·학력·고용형태·급여·기술·수집 검색어·마감일을
  앱 안의 상세 창에서 보고, 필요할 때만 원문으로 이동한다.
- 후보자 자료나 API 키는 웹에 내보내지 않으며 기존과 동일하게 소유자만 접근한다.
- 채용공고와 경진대회·공모전을 탭으로 전환해 같은 웹앱에서 탐색한다.
- 상세 창을 열면 DB에 저장된 원문 URL에서 주요업무·자격요건·우대사항을 읽어
  항목별로 표시하고 성공 결과를 로컬 캐시에 보관한다.
- 공부 주제는 원문 구획에서 실제 발견한 기술에만 연결하며, 자격증은 원문에 이름이
  명시된 경우에만 표시한다. 구획이나 기술을 못 찾으면 일반 추천을 지어내지 않고
  분석 불가 상태를 그대로 알린다.
- 기존 기업 분석 상태 파일이 있는 회사는 상세 화면에서 기업 분석 페이지로 연결한다.
- 기업 분석은 Notion을 열지 않고 앱에서 직접 읽는다. 모든 회사에 대해 수집 DB의
  관련 공고 수·최근 채용 활동·반복 요구 기술·최근 모집 직무를 즉시 집계한다.
- 기업분석 파이프라인이 새 경영 분석을 생성하면 기존 Notion 발행과 함께 전체
  분석 본문을 회사별 로컬 상태에 저장하여 앱의 `기업 경영 분석 전체 읽기`에서
  표시한다. 기존 분석은 다음 갱신부터 본문이 채워진다.
...
### 2026-09-10 — 웹앱 원문 분석에도 화면 OCR 연결

- 웹 대시보드의 `career-source-analysis`도 이제 정적 HTML에 주요업무·자격요건·우대사항 구획이 없거나 요청 자체가 실패하면 같은 화면 폴백을 실행한다. 이전에는 CLI 분석만 폴백을 사용하고 웹 API는 `urllib` 결과를 그대로 캐시해서, 이미지형·JS 공고가 계속 “수집할 수 없음”으로 보였다.
- Playwright가 렌더링한 DOM에 본문이 있으면 이를 먼저 사용하고, 그래도 없을 때만 전체 화면을 캡처한다. 캡처 이미지는 읽기 전용 Codex 비전으로 먼저 판독하고 실패할 때 Claude Read로 전환한다.
- 비전 결과에 실제 채용 구획이 확인된 경우에만 성공으로 저장한다. 실패 결과나 구획 없는 HTML은 캐시하지 않아, 일시 장애가 풀린 뒤 다시 열면 재수집할 수 있다. 로그인·CAPTCHA·robots.txt 제한은 우회하지 않는다.
