...
## 원문 추출 보정과 인사담당자 상담 버튼 (2026-09-12)

- 사람인 공고 섹션 제목 앞의 이모지·장식 문자를 허용해 실제 주요업무·자격요건·
  우대사항이 HTML에 있어도 화면 OCR로 불필요하게 넘어가던 문제를 고쳤다.
- 공고 상세에서 `담당자와 상담하기`를 누르면 선택 공고만 `이직 준비방`의
  인사담당자에게 전달한다. 인사담당자는 공고 설명, 면접 준비, 자소서 팁을
  채팅으로 제공하며 다른 페르소나를 동시에 호출하지 않는다.
...
- **원문 기반 공부 계획 구체화(2026-09-10)**: 단순 기술명과 매칭 근거 한 단어 대신, 원문에서 확인된 각 기술마다 `학습 방법`과 `완성할 결과물`을 구체적으로 보여준다. 근거 문자열은 화면에서 숨기지만 어떤 학습 주제를 고를지 판정하는 내부 근거로는 그대로 사용하며, 원문에 없는 기술을 임의로 끼워 넣지 않는다.
- **원문 핵심 조건 색상 강조(2026-09-10)**: 주요업무·자격요건·우대사항을 펼치면 Python·SQL·API·클라우드·AI 등 기술은 초록색, 필수·우대·경력 연차·학위·자격증 조건은 주황색으로 강조한다. HTML 이스케이프 뒤 허용된 표현만 `<mark>`로 감싸므로 원문 HTML이 실행되지 않는다.
- **파생 추천 자격증(2026-09-10)**: 원문에 자격증이 직접 적혀 있지 않아도 주요업무·자격요건·우대사항에서 확인된 기술·직무 신호로 관련성이 높은 자격증을 최대 3개 추천하고 이유를 함께 표시한다. 원문 명시 자격증과는 별도 구획이며 모든 추천에 `필수 아님`을 표시한다. 원문에 이미 나온 자격증은 추천 목록에서 중복하지 않는다.
...
### 2026-09-10 — 하루 신규 공고 20건 상한과 적합도 학습 선별

- 기존 DB 2,465건을 점검한 결과 2,214건이 적합도 0점이었고, 이 중 2,049건은 알바천국 사이트맵에서 들어온 무관 공고였다. 따라서 기존 숫자는 실제 지원 후보 수가 아니라 원천 후보를 거의 그대로 저장한 결과였다.
- 수집기는 여러 출처에서 후보를 넓게 확인하되 활성 DB에는 적합도 기준(`min_collection_score`, 기본 1점)을 통과한 신규 공고만 **하루 최대 20건** 저장한다. 같은 날 수동으로 여러 번 실행해도 `collection_daily_stats` 누적값으로 총 20건을 넘지 않는다. 기존 적합 공고의 내용·마감 갱신은 신규 상한과 별도로 유지한다.
- 신규 선별 점수는 기본 키워드 적합도, 비식별 후보자 프로필의 검증된 기술과의 교집합, 검색어별 과거 평균 점수·선정률을 합산한다. 학습 보너스는 최대 8점으로 제한해 과거 검색어 편향이 기본 적합도를 압도하지 않게 했다.
- `collection_query_stats`에 검색어별 실행 횟수·후보 수·선정 수·점수 합계를 누적한다. 같은 회사는 한 번 수집에서 최대 2건만 뽑아 특정 회사가 20개 슬롯을 독점하지 않게 한다.
- 매일 자동 정리에서 적합도 기준 미달 공고도 `expired_items.db`에 원본을 보관한 뒤 활성 목록에서 제외한다. `config.example.json`의 `max_new_jobs_per_run`과 `min_collection_score`로 정책을 조정할 수 있다.
- 수집·정리·추천 분석이 모두 끝나면 Shift Alarm이 이직 준비방의 `스터디코치` 한 명에게 전체 공고 종합 계획을 요청한다. 워커는 AI에게 DB 쓰기 권한을 주지 않고 활성 공고 최대 500건에서 반복 기술, 검색 직무축, 마감 명시 상위 공고를 결정론적으로 집계해 읽기 전용 상태로 전달한다.
- 스터디코치는 반복 요구 기술을 우선해 4주 목표와 이번 주 일별 계획을 채팅으로 보고한다. 하루 1~2개 과제, 학습 내용, 직접 만들 결과물, 완료 기준을 포함하고 실제 캘린더·파일·Notion은 수정하지 않는다.
...
### 2026-09-12 — 마감일 파싱 강화 + 웹 화면 즉시 필터링·추천순 정렬

**사용자 지적**: "경진대회 시간지난것들이 왜 아직도 떠있지? 기간지난건 알아서 삭제하는 기능 강화하고 최신공고 순으로 적합도높은순으로 필터링해서 보이게 해줘 구인공고도 마찬가지로 손봐줘."

- **원인 점검**: 실측 결과 `contests.db` 197건 중 20건이 마감일 텍스트를 아예 못 알아봐 영구 보존 상태였다. 대부분 콘테스트코리아류 2자리 연도 점 표기(`'26. 3. 26.(목) ~ 4. 24.(금)`)와 한글 년월일 표기(`2026년 6월 17일(수) ~ 7월 12일(일)`)였고, 둘 다 여러 달 전에 이미 마감된 대회였다.
- `job_collector.py`/`contest_collector.py`의 마감일 파서에 위 두 형식과 "A ~ B" 범위에서 뒷부분(B)에 연도가 없을 때 앞부분에서 찾은 연도를 이어받는 로직을 추가했다. 빈 날짜·`상시`·`채용시`·`수시`·달 단위로만 적힌 표기(`'26년 4월 중`)처럼 정확한 날짜를 가늠할 수 없는 항목은 여전히 삭제 대상으로 보지 않는다(기존 "모르면 배제하지 않는다" 원칙 유지). 실행 결과 경진대회 12건이 새로 정리 대상으로 잡혀 보관 후 삭제됐다.
- **웹 화면 즉시 필터링**: 기존엔 정리 배치가 하루 1회+7일 유예로 도는 동안 이미 마감된 항목이 화면에는 계속 보였다. 툴파챗 `/api/career-jobs`·`/api/career-contests`에 같은 마감 판정 로직을 이식해 마감 지난 항목을 요청마다 즉시 화면에서 숨기도록 했다 — DB 삭제(정리 배치·7일 유예)와 화면 노출(즉시)을 분리해, 실수로 지운 데이터를 복구할 여유는 그대로 유지된다.
- **정렬 기본값 변경**: 기본 정렬을 "최근 확인 순"에서 "추천순(최신·적합도)"(`score DESC, last_seen_at DESC`)으로 바꿨다. 웹 대시보드(`web_dashboard/index.html`)의 정렬 선택 목록 첫 항목도 "추천순(최신·적합도)"으로 바꿔 기본 선택되게 했다.
- 회귀 테스트: `test_job_collector.py`에 2자리 연도 점 표기·한글 년월일 표기 파싱 테스트를 추가했다(`test_job_deadline_parser_supports_2digit_year_dot_and_korean_particle_formats`, `test_contest_deadline_parser_supports_2digit_year_dot_and_korean_particle_formats`). 전체 29건 통과.
