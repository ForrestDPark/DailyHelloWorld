...
- Notion 발행은 `job_collector.py`와 같은 "🎴 이직시스템" 페이지 밑, 공고/공모전 고유키(source:source_id)마다 자기 페이지를 갖고 재분석 시엔 그 페이지만 갱신하는 방식을 재사용(`_notion_publish`가 두 파일에 거의 동일하게 존재 — 도메인별 파일 분리 원칙을 지키려고 공용 모듈로 합치지 않음). ★ 2026-08-18 전에는 `top_job_state_path`/`top_contest_state_path`가 카테고리로만 키잉되어 있어 같은 카테고리의 서로 다른 회사·공모전이 페이지 하나를 계속 돌려썼다 — 자세한 내용은 아래 버그 수정 항목 참고.
...
**★ 2026-08-09 최상위 "🎴 이직시스템" 추천 인덱스** — 맨 위의 `📋 최근 추천 기록` 토글(`TOP_INDEX_TOGGLE_ID`)에서 `🎯 오늘의 추천 공고`, `🏆 오늘의 추천 경진대회`, `🏢 기업 경영 분석 목록`을 서로 분리해 최신순 한 줄 목록으로 쌓는다. 추천 항목은 제목 앞에 `(점수 48)` 형식으로 점수를 표시하며, 새 점수 체계 도입 전에 저장돼 복원할 수 없는 항목만 `(점수 미기록)`으로 표시한다. 상세 분석 전문은 각 하위 페이지에만 둔다. 로컬 `data/top_index_history.json`을 원본으로 삼고, 이 워크스페이스에서 지원되지 않는 Notion `after` 파라미터는 쓰지 않는다. 순서를 바꿀 때는 토글의 기존 children을 삭제한 뒤 전체 목록을 원하는 순서로 다시 작성한다. 과거에 `kind=job`으로 함께 저장된 경진대회 이력은 제목의 🏆 표시로 자동 분류한다.
...
**★ 2026-08-18 "오늘의 추천 공고/경진대회" 링크 중복 버그 발견·수정**: `top_job_state_path(category)`/`top_contest_state_path(category)`가 카테고리 이름만으로 상태 파일 경로를 만들어서, 같은 카테고리(예: career) 안에서 회사가 바뀌어도 `_notion_publish`가 매번 "저장된 페이지가 있으면 갱신"하는 로직 때문에 페이지 하나(가장 최근 회사 것)를 계속 덮어썼다. 그 결과 최상위 페이지의 `📋 최근 추천 기록` 인덱스에 쌓인 과거 항목들의 링크가 전부 그 시점 이후 가장 최근 회사 페이지로 잘못 연결됨(사용자 리포트: "링크보니까 전부 한중 어쩌구로 연결되있는데" — 실측으로 career 14건, parttime 5건, 일반 공모전 9건, AI 경진대회 2건이 각각 페이지 하나로 뭉쳐 있었음). 두 함수 모두 카테고리 + 공고/공모전 고유키(`source:source_id`)로 상태 파일을 키잉하도록 고쳤다(공식 회사 분석 페이지 `company_profile.py`가 회사명으로 이미 하던 방식과 동일한 패턴). 이미 페이지가 겹쳐써져 사라진 과거 상세 내용은 복구할 수 없어, 인덱스의 과거 링크는 회사별로 남아있는 "🏢 기업 경영 분석" 페이지로 재연결했고(공모전은 대체 페이지가 없어 링크를 해제하고 소실 표시만 남김), 새로 분석하는 공고부터는 이 버그가 재발하지 않는다.
