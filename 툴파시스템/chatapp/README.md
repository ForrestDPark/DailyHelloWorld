...
- 화면 진입 시 `/api/whoami`로 로그인 상태를 확인하고, 로그인 안 된 방문자는
  ...
  **누구나 자유롭게** 할 수 있다(아이디 2~20자 한글/영문/숫자/밑줄, 비밀번호
  ...
  방(1:1 대화 포함)은 계정과 무관하게 전부 공유된다 — 예전처럼 방 하나를
  여러 사람이 같이 보고 같이 쓰는 구조(사용자가 직접 고른 정책).
...
실제 부작용이 있는 유일한 기능은 아래 "손동주 — 맥북 파일 정리 에이전트"의
파일 이동/삭제 실행이라, 그 승인 체크를 소유자 전용으로 좁혔다:

- `worker/persona_worker.py`의 `_maybe_execute_pending_plan`은 "방의 마지막
  사람 메시지"가 아니라 **`CHATAPP_OWNER_USERNAME` 환경변수와 정확히 일치하는
  sender의 메시지**만 승인으로 인정한다. 같은 방에서 다른 계정이 "승인"/
  "진행"이라고 써도 무시되고(계획만 취소), 오직 소유자 계정의 메시지만
  실행을 트리거한다 — AI가 판단하는 게 아니라 서버가 이미 DB에 저장해둔
  로그인 아이디 문자열을 코드로 정확히 비교하는 방식이라 프롬프트 인젝션으로
  우회할 수 없다.
