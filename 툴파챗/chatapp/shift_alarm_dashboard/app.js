...
function recurrenceText(recurrence){if(!recurrence)return"기존 규칙";const n=Number(recurrence.interval)||1;return recurrence.unit==="daily"?"매일":`${n}${recurrence.unit==="days"?"일":recurrence.unit==="weeks"?"주":"개월"}마다`}
function reminderBody(label,time,unit,interval,anchor,enabled=true){return{label,time,recurrence_unit:unit||null,recurrence_interval:Number(interval)||1,recurrence_anchor:anchor||null,enabled}}
...
function render(data){currentStatus=data;$("summary").replaceChildren(
 ...
 const profileSelect=$("time-profile");if(!timeProfileInitialized){const preferred=data.reminder_time_profile||data.shift||"시각";if([...profileSelect.options].some(option=>option.value===preferred))profileSelect.value=preferred;timeProfileInitialized=true}const profile=profileSelect.value;const schedule=$("schedule");schedule.replaceChildren();(data.reminder_schedule||[]).forEach(item=>{const row=document.createElement("tr"),labelCell=document.createElement("td"),label=document.createElement("input"),timeCell=document.createElement("td"),input=document.createElement("input"),repeatCell=document.createElement("td"),enabledCell=document.createElement("td"),enabled=document.createElement("input"),action=document.createElement("td"),save=document.createElement("button"),remove=document.createElement("button"),repeat=recurrenceEditor(item);label.type="text";label.maxLength=80;label.value=item.label;label.disabled=item.editable===false;labelCell.append(label);input.type="time";input.value=(item.times||{})[profile]||item.time||"";input.setAttribute("aria-label",`${item.label} ${profile} 시각`);input.disabled=item.editable===false;timeCell.append(input);repeatCell.append(repeat.box);enabled.type="checkbox";enabled.checked=item.enabled;enabled.disabled=item.editable===false;enabled.setAttribute("aria-label",`${item.label} 사용 여부`);enabledCell.append(enabled,document.createTextNode(item.editable===false?" 자동":enabled.checked?" 켜짐":" 꺼짐"));save.type="button";save.textContent="저장";remove.type="button";remove.textContent="영구 삭제";remove.className="danger-button";if(item.editable===false){save.disabled=true;remove.disabled=true;repeat.box.replaceChildren(document.createTextNode("자동 계산"))}else{save.addEventListener("click",async()=>{save.disabled=true;try{if(profile!=="시각"&&!item.custom)await api("/api/shift-alarm/reminder-time",{method:"PUT",body:JSON.stringify({label:item.label,time:input.value,profile})});const baseTime=profile==="시각"||item.custom?input.value:item.time;await api(`/api/shift-alarm/reminder-definitions/${encodeURIComponent(item.key)}`,{method:"PUT",body:JSON.stringify(reminderBody(label.value,baseTime,repeat.unit.value,repeat.interval.value,repeat.anchor.value,enabled.checked))});notice(`${label.value} 설정을 저장했습니다.`);await load()}catch(e){notice(e.message,true)}finally{save.disabled=false}});remove.addEventListener("click",()=>deleteReminderDefinition(item))}action.append(save,remove);row.append(labelCell,timeCell,repeatCell,enabledCell,action);schedule.append(row)});if(!(data.reminder_schedule||[]).length){const row=document.createElement("tr"),cell=document.createElement("td");cell.colSpan=5;cell.className="empty";cell.textContent="등록된 리마인더가 없습니다.";row.append(cell);schedule.append(row)}
