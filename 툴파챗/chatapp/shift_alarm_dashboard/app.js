...
function openReminderEditor(item,profile){
 ...
 label.value=item.display_label||item.label;time.value=(item.times||{})[profile]||item.time||"";label.disabled=time.disabled=item.editable===false;unit.replaceChildren();
 [["",`기본 · ${item.default_recurrence_text||"자동 계산"}`],["daily","매일 (1일에 1회)"],["days","일수 간격"],["weeks","주수 간격"],["months","개월 간격"]].forEach(([value,text])=>unit.add(new Option(text,value)));
 unit.value=item.recurrence?.unit||"";interval.value=item.recurrence?.interval||1;anchor.value=item.recurrence?.anchor||new Date().toISOString().slice(0,10);enabled.checked=item.enabled;unit.disabled=interval.disabled=anchor.disabled=enabled.disabled=item.editable===false;
 const refresh=()=>detail.classList.toggle("hidden",!unit.value||unit.value==="daily");unit.onchange=refresh;refresh();
 $("reminder-edit-save").disabled=item.editable===false;$("reminder-edit-delete").disabled=item.editable===false;
 $("reminder-edit-save").onclick=async()=>{const button=$("reminder-edit-save");button.disabled=true;try{if(profile!=="시각"&&!item.custom)await api("/api/shift-alarm/reminder-time",{method:"PUT",body:JSON.stringify({label:item.label,time:time.value,profile})});const baseTime=profile==="시각"||item.custom?time.value:item.time;await api(`/api/shift-alarm/reminder-definitions/${encodeURIComponent(item.key)}`,{method:"PUT",body:JSON.stringify(reminderBody(label.value,baseTime,unit.value,interval.value,anchor.value,enabled.checked))});dialog.close();notice(`${label.value} 설정을 저장했습니다.`);await load()}catch(e){notice(e.message,true)}finally{button.disabled=false}};
...
function renderReminderSchedule(items,profile){const schedule=$("schedule");schedule.replaceChildren();items.forEach(item=>{const row=document.createElement("tr"),name=document.createElement("td"),title=document.createElement("b"),state=document.createElement("small"),time=document.createElement("td"),repeat=document.createElement("td"),action=document.createElement("td"),edit=document.createElement("button");title.textContent=item.display_label||item.label;state.textContent=item.enabled?"켜짐":"꺼짐";name.append(title,state);time.textContent=(item.times||{})[profile]||item.time||"—";repeat.textContent=item.recurrence?recurrenceText(item.recurrence):`기본 · ${item.default_recurrence_text||"자동 계산"}`;edit.type="button";edit.textContent="수정";edit.disabled=item.editable===false;edit.addEventListener("click",()=>openReminderEditor(item,profile));action.append(edit);row.append(name,time,repeat,action);schedule.append(row)});if(!items.length){const row=document.createElement("tr"),cell=document.createElement("td");cell.colSpan=4;cell.className="empty";cell.textContent="등록된 리마인더가 없습니다.";row.append(cell);schedule.append(row)}}
