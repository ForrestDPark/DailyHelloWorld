...
function appendLinkifiedText(container, text) {
  ...
  for (const match of text.matchAll(urlPattern)) {
    ...
    if (suffix) appendTextWithBreaks(suffix);
    cursor = match.index + match[0].length;
  }
  if (cursor < text.length) appendTextWithBreaks(text.slice(cursor));
}
...
async function loadRoomUserMembers() {
  ...
  try {
    ...
    for (const username of members) {
      ...
      // 붙인다. 방 주인 본인 칩에는 안 붙인다(서버도 400으로 거부함).
      if (username !== owner_username) {
        ...
        kickBtn.addEventListener("click", async () => {
          ...
          try {
            ...
            if (e.message !== "unauthorized" && e.message !== "forbidden") alert("내보내기 실패");
          }
        });
        chip.appendChild(kickBtn);
      }
