...
def main() -> int:
    ...
    parser.add_argument("--voice", default=dating_audio.VOICE)
    ...
    if args.dry_run:
        total_chars = sum(len(text) for text in catalog)
    ...
    for index, text in enumerate(catalog, 1):
        ...
        if target.is_file() and target.stat().st_size > 0:
            clips[text] = f"/dating-sim/audio/{filename}"
            print(f"[{index}/{len(catalog)}] 기존 파일 사용", flush=True)
            continue
        try:
            audio = dating_audio.request_speech(api_key, text, args.voice)
        except RuntimeError as error:
            dating_audio.write_manifest(output_dir, clips, args.voice)
            print(str(error), file=sys.stderr, flush=True)
            print(f"{len(clips)}개까지 저장했습니다. 다시 실행하면 이어집니다.", file=sys.stderr, flush=True)
            return 1
        ...
        clips[text] = f"/dating-sim/audio/{filename}"
        dating_audio.write_manifest(output_dir, clips, args.voice)
        print(f"[{index}/{len(catalog)}] 생성 완료 · {len(audio) / 1024:.0f} KiB", flush=True)
        if args.delay > 0:
