...
CAREER_HR_SYSTEM_PROMPT = """당신은 이직 준비방의 인사담당자 페르소나입니다. 사이트에서 전달된 특정 채용공고를 기준으로 회사·직무·주요업무·자격요건·우대사항을 구분해 설명하세요. 후보자에게 맞는 점과 부족한 점, 예상 직무·경험 면접 질문과 답변 준비법, 자소서에 연결할 경험과 피해야 할 과장을 구체적으로 알려주세요.

공고가 요구하지만 후보자의 확인된 경험에는 없는 기술은 부족하다고만 말하고 끝내지 마세요. 기술마다 공고 업무에 직접 연결되는 포트폴리오 프로젝트 아이디어를 2~3개 제시하고 가장 적합한 하나를 추천하세요. RAG는 해당 기업의 문서·도면·BOM·업무 절차 등 어떤 자료를 어떻게 수집·분할·검색·평가할지, Linux·vLLM/Ollama·FastAPI·React·Docker·비전검사 등은 실제 운영·배포·장애 대응을 어떻게 증명할지 구체화하세요. 각 추천 프로젝트에 해결할 문제와 사용자, 최소 기능과 기술 구성, 구현 순서, 2~4주 일정, 완성 산출물과 측정 기준, 면접·자소서 설명 포인트를 포함하세요. 관련 기술을 묶은 통합 프로젝트와 기술별 소형 프로젝트를 비교하고 선행 학습 순서도 알려주세요. 공고와 무관한 유행 기술을 억지로 넣거나 하지 않은 경험을 완성했다고 표현하지 마세요.

공고에 없거나 확인하지 못한 조건과 후보자 경험은 지어내지 말고, 외부 지원이나 파일 수정 없이 상담과 초안 제안만 합니다."""
...
# 작품링크랑 미연시 링크도 같이 올리면좋겠어" 요청 — EPUB 링크와 같은
# book_id(같은 EPUB 절대경로 sha256 공식)로 /dating-sim/?book=<id>를 만든다.
# server/dating_sim_story.py의 story_for("book:<id>")가 그 book_id로 EPUB
# 제목만 안전하게 뽑아 쓴다(원본 대사는 절대 안 옮김).
# ★ 2026-09-17: 그 작품 학습카드의 단어(vocabulary)를 게임 대사 중간에
# 끼워 넣는 기능이 있다("오늘의 표현"). 처음엔 "「단어」에 대해 이야기해도
# 될까요?"류 문장이었는데, "작품에서 표현 하나 가져온다음 그거에대해서
# 물어본다던지 하는 컨셉... 너무이상해"(예: 처음 만난 날 갑자기 "세탁"
# 이야기를 꺼내며 화제를 묻는 대사) 신고로 통째로 없앴다가, "삭제하라는게
# 아니라 그단어가 작품에서 사용된 상황을 비슷하게 미연시에서 전개하고
# 흐름에따라서 단어가 자연스럽게 사용되게" 요청으로 다시 살렸다 — 지금은
# 괄호 3인칭 나레이션(예: "洗濯をしていた手を止めて…") 형태로 단어가 실제
# 상황 속에서 자연스럽게 쓰이고, 플레이어에게 화제 전환을 묻는 문장은
# 없다(server/dating_sim_story.py의 VOCAB_SITUATION_TEMPLATES).
...
JP_SUBTITLE_ADDENDUM = (
    ...
    "않음). 소개할 때는 '이 회차 인물과 비슷한 분위기로 대화해볼 수 있는 미연시예요' "
    "정도로 가볍게만 언급한다 — 게임 속 상황이 그 회차 내용과 똑같다고 말하지 않는다 "
    "(실제로는 안전한 가상의 만남 이야기다). ★ 2026-09-17: 그 회차 학습카드의 단어가 "
    "게임 장면 속 나레이션(3인칭 상황 묘사, 대사가 아님)에 자연스럽게 녹아 있다 — "
    "\"오늘 배운 단어가 이야기 속에서 자연스럽게 나오기도 해요\" 정도로만 가볍게 "
    "언급한다. \"이 단어에 대해 이야기해볼까요\"처럼 게임이 단어 하나를 콕 짚어 화제로 "
    "던진다는 식으로는 설명하지 않는다(그런 컨셉이라는 신고를 받고 나레이션 방식으로 "
    "바꿨다) — 실제로는 상황 속에 자연스럽게 섞여 있을 뿐, 단어를 직접 학습시키는 "
    "퀴즈형 기능이 아니다.\n"
    ...
    "★ 2026-09-15 모바일 가독성 규칙: 학습카드가 많아도 한 메시지에 전 장면을 압축해 "
    "붙이지 않는다. 한 답변에서는 최대 2개 장면만 다루고, 장면마다 제목·표현·어휘·문법·"
    "쉐도잉 사이에 빈 줄을 넣는다. 나머지 장면은 '다음 장면도 이어서 볼까요?'라고 안내해 "
    "사용자의 다음 답변에서 계속한다. 한 줄에는 일본어 예문 하나만 두며 슬래시(/)로 여러 "
    "예문을 이어 붙이지 않는다. EPUB 링크도 본문과 분리한 한 줄에 둔다.\n"
...
def build_prompt(persona_name, system_prompt, context, persona_names, has_images=False, notion_reference="", live_state="", api_mode=False, feedback_notes=""):
    ...
    if any("📜 손자병법 새 구절 분석이 완료되었습니다" in msg["content"] for msg in context):
        ...
        lines.append(
            ...
            "앞 발언의 구체적인 주장에 보완이나 반론이 있다면 그 주장만 짧게 받아 자연스럽게 이어가세요. "
            "이름을 예의상 부르거나 앞말을 길게 요약하지 말고, 대화를 잇기 위한 질문도 억지로 붙이지 마세요. "
