...
# ★ 2026-09-17: 처음엔 그 작품 학습카드의 단어(vocabulary)를 새로 쓴 안전한
# 문장에 끼워 넣어 "오늘의 표현"을 만들었는데, "작품에서 표현 하나
# 가져온다음 그거에대해서 물어본다던지 하는 컨셉... 너무이상해"(예: 처음
# 만난 날 갑자기 "세탁"이라는 단어 이야기를 꺼냄) 신고로 그 기능을 완전히
# 없앴다 — 지금은 그 작품 분위기만 참고한 순수 가상 만남이고, 표현 학습
# 기능은 없다.
...
JP_SUBTITLE_ADDENDUM = (
    ...
    "(실제로는 안전한 가상의 만남 이야기다). ★ 2026-09-17: 예전엔 그 회차 학습카드의 "
    "단어를 게임 대사에 끼워 넣었는데(\"오늘 배운 표현으로 대화해볼 수 있는\"류 문구는 "
    "이 기능 전제였다), \"작품에서 표현 하나 가져온 다음 그거에 대해서 물어본다던지 "
    "하는 컨셉... 너무이상해\" 신고로 그 기능 자체를 없앴다 — 이제 표현 학습 도구가 "
    "아니라 순수 분위기 참고용 미연시이므로 표현 학습을 약속하는 문구는 쓰지 않는다.\n"
