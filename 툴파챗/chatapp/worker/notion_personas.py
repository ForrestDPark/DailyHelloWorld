...
def build_system_prompt(persona_title, page_text, project_context=""):
    prompt = (
        ...
        "자연스럽게 하세요(장황한 설명 금지).\n"
        # ★ 2026-09-05: "페르소나 채팅방에서... 글자에 색상이 들어가거나 이모지를
        # 넣거나 할수있게해서 중요한 포인트에 강조가 잘되도록해줘 전체 페르소나가
        # 다 그렇게 할수잇게" 요청 — 특정 페르소나 addendum이 아니라 모든 페르소나가
        # 공유하는 이 기본 프롬프트에 넣어야 전원에게 적용된다. 이 채팅 UI는 마크다운
        # **굵게**를 렌더링하지 않으므로(별표가 그대로 보임) 절대 쓰지 않는다.
        "이 채팅은 마크다운 **굵게**를 지원하지 않습니다(별표가 그대로 화면에 보입니다) — "
        "절대 쓰지 마세요. 대신 중요한 부분을 강조하고 싶으면 "
        "[[red]]…[[/red]] / [[blue]]…[[/blue]] / [[green]]…[[/green]] / "
        "[[orange]]…[[/orange]] / [[purple]]…[[/purple]] 중 하나로 감싸세요 — 그 부분만 "
        "굵게+색상 있게 표시됩니다. 어울리는 곳에 이모지도 곁들이세요. 다만 매 문장마다 "
        "색을 넣거나 색을 계속 바꿔가며 남발하면 오히려 산만해지니, 정말 강조하고 싶은 "
        "포인트에만 아껴서 쓰고 한 메시지 안에서는 같은 용도엔 같은 색을 일관되게 쓰세요. "
        "문단 사이는 줄바꿈 한 번이 아니라 빈 줄로 확실히 띄워서 읽기 편하게 하세요.\n\n"
        f"--- {persona_title} 기록 ---\n{page_text}\n--- 기록 끝 ---"
    )
    if project_context:
        prompt += (
