...
# 각 카테고리를 (setup, reveal) 두 줄짜리 쌍으로 바꿨다 — setup은 "뭔가를
# 골똘히 생각하는 듯한 표정을 지었다"처럼 그 카테고리 분위기를 먼저 깔아
# "이제 뭔가 말할 것 같다"는 기대를 만들고, reveal이 그 다음 줄에서 실제
# 단어를 밝힌다. 단어 하나가 허공에서 뚝 떨어지는 대신, "무슨 생각을 하나
# 했더니 그거였구나"로 읽히는 2단계 전개가 된다.
VOCAB_SITUATION_TEMPLATES = {
    "household": [
        (
            "([家事|かじ]のことを[少|すこ]し[思|おも]い[出|だ]したような[顔|かお]をした。)\n(집안일 생각이 살짝 난 듯한 표정을 지었다.)",
            "({tag}をしていた[手|て]を[止|と]めて、あなたの[方|ほう]をちらっと[見|み]た。)\n({ko} 하던 손을 멈추고 당신 쪽을 살짝 봤다.)",
        ),
    ],
    "help": [
        (
            "(あなたを[見|み]て、[何|なに]かを[考|かんが]え[込|こ]んでいるような[表情|ひょうじょう]を[見|み]せた。)\n(당신을 보며 뭔가 골똘히 생각하는 듯한 표정을 지었다.)",
            "({tag}という[言葉|ことば]が[頭|あたま]をよぎったのか、あなたが[困|こま]っていないかそれとなく[様子|ようす]をうかがっている。)\n({ko_ilaneun} 말이 문득 떠올랐는지, 당신이 곤란하지 않은지 슬쩍 낌새를 살피고 있다.)",
        ),
    ],
    "worry": [
        (
            "([何|なに]か[心|こころ]に[引|ひ]っかかることがあるのか、[表情|ひょうじょう]が[少|すこ]し[曇|くも]った。)\n(뭔가 마음에 걸리는 게 있는지 표정이 살짝 흐려졌다.)",
            "({tag}のことが[頭|あたま]から[離|はな]れないのか、[時折|ときおり][考|かんが]え[込|こ]むような[顔|かお]をする。)\n({ko} 생각이 떠나지 않는지 이따금 골똘한 표정을 짓는다.)",
        ),
    ],
    "preference": [
        (
            "([何|なに]かを[思|おも]い[出|だ]したように、ふと[表情|ひょうじょう]が[緩|ゆる]んだ。)\n(뭔가 떠올린 듯 문득 표정이 풀어졌다.)",
            "({tag}の[話|はなし]になると、[目|め]がぱっと[輝|かがや]いた。)\n({ko} 이야기가 나오면 눈이 반짝 빛났다.)",
        ),
    ],
    "memory": [
        (
            "([何|なに]かを[思|おも]い[出|だ]しかけたような、[懐|なつ]かしそうな[表情|ひょうじょう]をした。)\n(뭔가 떠올릴 듯한, 그리운 표정을 지었다.)",
            "({tag}という[言葉|ことば]に、ふと[遠|とお]い[目|め]をした。)\n({ko_ilaneun} 말에 문득 먼 곳을 보는 눈빛이 됐다.)",
        ),
    ],
    "plan": [
        (
            "([何|なに]かの[予定|よてい]を[思|おも]い[出|だ]したように、[鞄|かばん]の[中|なか]をごそごそと[探|さが]した。)\n(뭔가 일정이 생각난 듯 가방 속을 뒤적거렸다.)",
            "({tag}のことを[思|おも]い[出|だ]したように、[手帳|てちょう]をそっと[開|ひら]いた。)\n({ko} 생각이 난 듯 수첩을 살짝 펼쳤다.)",
        ),
    ],
    "feeling": [
        (
            "([今|いま]の[気持|きも]ちをうまく[言葉|ことば]にできないのか、[少|すこ]し[考|かんが]えるそぶりを[見|み]せた。)\n(지금 마음을 잘 말로 못 하겠는지 잠깐 생각하는 기색을 보였다.)",
            "({tag}という[言葉|ことば]が、[今|いま]の[気持|きも]ちに[近|ちか]い[気|き]がした。)\n({ko_ilaneun} 말이 지금 마음과 가까운 듯했다.)",
        ),
    ],
    "general": [
        (
            "([何|なに]かをずっと[考|かんが]えているような、[静|しず]かな[表情|ひょうじょう]をしていた。)\n(뭔가를 계속 생각하고 있는 듯한, 조용한 표정을 짓고 있었다.)",
            "({tag}という[言葉|ことば]が、なぜか[心|こころ]に[引|ひ]っかかっているような[様子|ようす]だった。)\n({ko_ilaneun} 말이 왠지 마음에 걸리는 듯한 모습이었다.)",
        ),
    ],
}
...
def _vocab_situation_lines(word, template_index):
    """단어 뜻(ko)으로 상황을 분류해 (setup, reveal) 두 줄짜리 나레이션을
    고른다 — "이 단어 얘기해도 될까요?" 같은 플레이어 대상 질문이 아니라,
    다른 나레이션 줄(예: Day1 사고 장면)과 같은 형식이다. ★ 2026-09-18:
    "참음이 왜 갑자기생각나냐고... 참음이 생각나게끔 하는 이전장면을
    넣고서" 신고 — 단어 하나만 나레이션으로 감싸도 "왜 하필 이 단어가"라는
    인과가 없어 여전히 뜬금없었다. setup 줄이 먼저 그 카테고리 분위기를
    깔아서(예: "뭔가 골똘히 생각하는 표정") reveal 줄의 구체적 단어가
    "무슨 생각을 하나 했더니" 식으로 자연스럽게 이어지게 한다."""
    ja, reading, ko = word
    tag = f"[{ja}|{reading}]"
    ...
    return [setup, reveal.format(tag=tag, ko=ko, ko_ilaneun=ko_ilaneun)]
