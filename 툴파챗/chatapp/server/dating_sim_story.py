...
VOCAB_SITUATION_TEMPLATES = {
    ...
    # ★ 2026-09-18: "효과라는 말이 왜 갑자기 생각났냐고" 신고 — general은
    # 위 어떤 카테고리에도 안 걸린 단어의 최후 보루라서, 특정 감정·상황을
    # 전제하면(예: 알바 스트레스를 참는 이야기) "효과" 같은 무관한 단어와
    # 부딪힌다. 그래서 general만은 어떤 뜻의 단어가 와도 의미가 안 부딪히게
    # "오늘 하루를 돌아보다가 문득"이라는 내용 중립적인 틀로 바꿨다 —
    # 구체성은 다소 낮아지지만, 특정 서사와 충돌해 뜬금없어지는 것보다는
    # 안전하다(카테고리를 못 찾은 단어에 한해서만 쓰이는 최후 수단이므로).
    "general": [
        (
            "[今日|きょう]も[色々|いろいろ]なことをしているうちに、いつの[間|あいだ]にか[夕方|ゆうがた]になっていました。\n오늘도 이것저것 하다 보니 어느새 저녁이 됐더라고요.",
            "そうしているうちに、ふと{tag}という[言葉|ことば]が、[今日|きょう]の[一日|いちにち]をよく[表|あらわ]している[気|き]がしました。\n그러다 문득 {ko_ilaneun} 말이, 오늘 하루를 잘 나타내는 것 같다는 생각이 들었어요.",
        ),
    ],
}
