...
DAY_BEATS = {
    ...
    # 의미가 이해가 안가... 고민내용에대해서 더 대화를 해야지" 신고 —
    # 원래는 "장래 문제로 고민 중"이라고만 말하고 바로 "이런 얘기 해도
    # 되냐"고 묻는 게 전부라 고민의 실체가 없었다. 무슨 고민인지(지금 일을
    # 계속할지 새 도전을 할지)를 한 줄 더 넣어 내용을 구체화했다.
    ...
    # ★ 2026-09-17: "갑자기대화가 끝나는식으로 되지않게" 요청 — 4일차와
    # 같은 이유로 6일차도 "서운했다"는 감정만 말하고 바로 다음 줄로 넘어가
    # 얕았다. 왜 서운했는지(혹시 내가 뭘 잘못 말했나 계속 신경 쓰였다는)
    # 구체적인 생각 한 줄을 더해 대화에 내용을 채웠다.
...
# ★ 2026-09-17: "장래에대한 고민인데 왜 돕다에대해서 이야기한다는거야 의미가
# 이해가 안가" 신고 — 4·6일차는 고민 상담·서운함 사과처럼 감정적으로 무거운
# 대화인데, 그 사이에 EPUB에서 뽑은 무관한 단어(예: "돕다")로 화제를 트는
# "오늘의 표현" 줄이 끼어들면 대화 톤이 갑자기 끊긴다. 이 두 요일은 표현
# 줄을 아예 건너뛴다 — 가벼운 요일(1·2·3·5·7)에서만 캐주얼한 곁가지로 쓴다.
DAYS_WITHOUT_VOCAB_ASIDE = {4, 6}
...
def _seven_day_scenes(location_lines, character_name_ko="소이", character_name_jp="ソイ", vocab_pool=None):
    ...
    for day, (lines, choices) in DAY_BEATS.items():
        scenes[day] = {}
        vocab_word = (
            vocab_pool[(day - 1) % len(vocab_pool)]
            if vocab_pool and day not in DAYS_WITHOUT_VOCAB_ASIDE else None
        )
