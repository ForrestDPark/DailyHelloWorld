...
//   medium — 왼쪽(근무·날씨·리마인더) / 오른쪽(AI 사용량) 2단
//   large  — medium 레이아웃 + 아래에 손자병법 최신 구절 + 이직시스템 요약 추가
// Mac 저장공간과 급여/시급은 iPhone 위젯에서 표시하지 않는다.
...
const COLOR_CODEX = new Color("#c9a7ff");
...
function buildRightColumn(stack, status) {
  ...
  if (status.codex_percent !== null && status.codex_percent !== undefined) {
    const color = status.codex_critical ? COLOR_WARN : COLOR_CODEX;
