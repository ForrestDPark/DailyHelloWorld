...
**★ 2026-08-11 일일 루틴 분리:** 하위 원본 페이지 `일일 루틴체크리스트`의 20개 항목을 원본 순서 그대로 `🌅 일일 루틴 체크리스트` 구역에 매일 생성한다. 근무표 조건에 따라 생기는 `🔔 오늘의 리마인더`와는 별도 구역이며, 리마인더가 0개인 날에도 루틴은 반드시 생긴다. 루틴 완료 여부는 메뉴바 타이틀에 표시하지 않는다. 기존 날짜 토글이 있으면 사용자가 체크한 리마인더를 덮어쓰지 않고 누락된 루틴 구역만 추가한다.
