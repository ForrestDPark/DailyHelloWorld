...
**사용자 요청**: "8.15일을 음력으로 계산하면 몇월몇일이야? 엄마생일인데 일주일전에 미리 리마인더 해주고 당일에 리마인더 알람 생성해줘."

- 2026년 음력 8월 15일(추석)은 양력 9월 25일(금)로 WebSearch로 복수 출처 교차 확인했다.
- 새 1분 주기 타이머 2개(`_check_mom_birthday_advance_reminder`/`_check_mom_birthday_alarm`)가 각각 9/18(일주일 전)·9/25(당일) 09:00에 하루 한 번 `notify_spoken()`으로 알림(화면 배너+음성)을 띄운다. 근무/휴무와 무관하게 항상 봐야 하는 개인 일정이라 기상 알람 시각이 아닌 고정 시각(09:00)을 쓴다.
- 음력 생일이라 해마다 양력 날짜가 바뀐다 — 지금은 2026년분 날짜만 직접 박아뒀고, 내년 이후에도 자동으로 이어가려면 음력→양력 변환 로직이 별도로 필요하다(요청 시 추가).
