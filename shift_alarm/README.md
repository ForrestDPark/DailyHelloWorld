...
- **AI 사용량 상태창 직접 표기 (★ 2026-08-05 추가, 2026-08-08 간소화)**: 드롭다운을 열지 않아도 바로 보이도록 타이틀 맨 끝에 글자 없이 `{Codex 주간%}%`(연보라), `{Claude 5시간%}%`(오렌지) 순서로 붙인다. 저장공간의 초록·비 오는 날의 파랑과 겹치지 않도록 Codex는 연보라로 구분한다. Codex는 **7일 한도의 하루 몫인 `100/7 ≈ 14.3%`를 다 쓰면**, Claude는 주간 윈도우가 90% 이상이면 해당 숫자가 빨간색으로 바뀐다. 값을 아직 못 가져왔으면 해당 숫자를 생략한다. 자세한 데이터 출처는 14번 항목 참조.
...
- **레이아웃은 `config.widgetFamily`로 위젯 크기에 따라 분기한다(★ 2026-08-06)** — Scriptable은 내용이 위젯 프레임을 넘으면 자동으로 줄여주지 않고 그냥 잘라버리므로, 크기별로 보여줄 내용 자체를 다르게 짰다:
  ...
  - **미디엄**: `addStack()` + `layoutHorizontally()`로 좌우 2단
    ...
    - 오른쪽: 오늘 급여(근무 중이면 지금까지 번 금액, 근무 전이면 예상 금액), 🪙 AI 사용량 — Codex는 주간 한도의 하루 몫(약 14.3%)을 다 쓰면 빨강, Claude는 주간 90% 이상이면 빨강(그 전에는 Codex=연보라/Claude=오렌지)
