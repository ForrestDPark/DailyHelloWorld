...
근무 시작 전 깨워주는 알람. macOS `launchd` plist(`~/Library/LaunchAgents/com.shfitalarm.music.plist`)로 등록되며, 울릴 때 `~/Library/Scripts/shift_alarm_run.sh`가 실행된다. 전날의 좋아요 재생 큐가 클래식과 섞이지 않도록 기존 Elmedia 프로세스를 완전히 종료하고 `Playlist.db`의 `playlist_items`/`item_order`만 비운다. 그 뒤 폴더 자체를 넘기지 않고 `Coffee and Meditation` 안의 실제 음원만 나열한 `~/.shift_alarm_playlists/classic.m3u8`을 열기 때문에 Elmedia의 `Nothing to open` 오류와 기존 목록 혼합을 함께 방지한다. 음악 원본은 삭제하지 않는다. 메뉴의 `🎬 Elmedia 지금 바로 재생`과 `⭐ 좋아요 플레이하기`도 각각 독립된 M3U8을 매번 다시 만들며, `open -n`을 쓰지 않아 Elmedia 복수 인스턴스도 만들지 않는다. 기상 알람에서는 "아침루틴음악재생" 단축어(유튜브 랜덤 음악)를 실행하지 않아 클래식만 재생한다(★ 2026-08-08, 2026-08-10 큐 DB 초기화 강화, 2026-08-11 명시적 M3U8 재생으로 변경).
...
- **`🔔 오늘: ...` 항목(★ 2026-08-07 개선, 2026-08-09 Notion 앱 열기)**: 예전엔 콜백이 없어서(클릭해도 아무 동작 안 함) NSMenu가 자동으로 회색/비활성으로 그렸다. `_build_reminder_status_menu_item()`이 이제 클릭 콜백(16-2의 "🎎 일일 체크리스트" Notion 페이지로 이동)을 달아서 일반 텍스트처럼 보이게 하고, 리마인더마다 `REMINDER_MENU_COLOR_CYCLE`(주황·파랑·보라·초록·빨강·노랑 순환)로 색을 입혀 한눈에 구분되게 했다. `make_open_url_callback()`은 Notion 도메인을 감지하면 `open -a Notion <URL>`로 데스크톱 앱을 명시하므로, 기본 브라우저가 Chrome이어도 Notion 앱에서 열린다. 일반 웹 링크는 기존처럼 기본 브라우저로 연다.
...
- **★ 2026-08-09 원인 파악·재시도 주기 개선**: `🪙 Claude`가 "확인 불가"로 뜨는 주된 원인은 키체인의 OAuth 액세스 토큰(수 시간짜리, 실측 약 8시간)이 만료됐기 때문 — 이 토큰은 `claude` CLI를 실제로 켜서 쓸 때만 갱신되므로, 밤새 켜지 않으면 아침에 만료 상태로 남는다. 이 토큰은 진짜 `claude` 로그인에 쓰는 공유 자격증명이라 shift_alarm이 직접 refreshToken으로 갱신하는 건 위험(리프레시 토큰이 1회용 로테이션이면 잘못 건드릴 시 실제 로그인이 깨질 수 있음)해서 하지 않는다. 대신 조회가 실패한 상태일 땐 재시도 주기를 12분(`AI_USAGE_NORMAL_INTERVAL`)에서 2분(`AI_USAGE_RETRY_INTERVAL`)으로 당겨서, 다음에 `claude`를 켜는 즉시(키체인 토큰이 갱신되는 즉시) 몇 분 안에 자동 복구되게 한다. 성공하면 다시 12분 주기로 되돌아간다.
...
**★ 2026-08-11 일일 루틴 분리:** 하위 원본 페이지 `일일 루틴체크리스트`의 최신 25개 항목과 단계 구분선 4개를 원본 순서 그대로 `🌅 일일 루틴 체크리스트` 구역에 매일 생성한다. 근무표 조건에 따라 생기는 `🔔 오늘의 리마인더`와는 별도 구역이며, 리마인더가 0개인 날에도 루틴은 반드시 생긴다. 루틴 완료 여부는 메뉴바 타이틀에 표시하지 않는다. 원본 템플릿이 바뀌면 오늘 루틴 구역만 최신 순서로 교체하며, 이름이 같은 항목의 체크 상태와 기존 리마인더는 보존한다.

생성 직전마다 원본 페이지(`3b932a1e-ae80-8021-912b-d160fe6cb629`)의 블록을 API로 다시 읽는다. 일반 문단은 체크 항목, 구분선은 루틴 단계 구분선으로 변환하고 전체 순서의 SHA-256 해시를 저장한다. 같은 날이라도 원본 해시가 바뀌면 오늘 루틴을 다시 동기화한다. 원본 조회가 실패하거나 항목이 비어 있으면 오래된 하드코딩 목록을 대신 올리지 않고 기록을 보류해 다음 실행 때 재시도한다.
