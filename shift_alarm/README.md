...
| 항목 | 조건 |
|---|---|
| 🏋️ 상체/하체 운동 하는 날 | 주 2회: 휴무 블록 **첫날 = 상체**, 그로부터 **이틀 뒤 = 하체**(보통 복귀 첫 근무일). 단, 그날 "근무 끝나고" 가는 시각에 헬스장이 닫혀있으면 스킵 (아래 6번 참조). ★ 2026-07-24: 예전엔 둘 다 그냥 "헬스장 가는 날"로 동일했는데 상체/하체로 구분함 |
| 📞 엄마한테 전화 | 휴무 블록 첫날 |
| 🧹 카톡 정리 | 휴무 블록 마지막날 |
| 🛍️ 아울렛 쇼핑 | 월 1회: 그 달의 **첫 번째** 휴무 블록 시작일 |
| 🚶 2만보 걷는 날 | 주 1회: 휴무 블록 **마지막날**. ★ 2026-07-24: "너무 안 뜬다"는 피드백으로 예전에 있던 "헬스장 날과 겹치면 건너뜀" 제약을 없앰 — 이제 헬스장 날과 겹쳐도 그냥 같이 뜬다 |
...
## 10-1. 🔄 북마크 최신화 (天 폴더) — krNN. 서브도메인 로테이션 자동 교체 (2026-07-23 추가)
...
5. 결과는 `rumps.notification`으로 몇 개 바꿨는지 알려줌. 번역: 특정 도메인의 현재 살아있는 번호를 못 찾으면(예: 사이트 자체가 완전히 다운) 그 도메인은 `failed_domains`로 보고하고 나머지는 정상 처리.
6. 네트워크 확인이 몇 초 걸릴 수 있어서 클릭 즉시 "확인 중..." 알림을 띄우고 `threading.Thread`로 백그라운드 실행 — 메뉴바 자체가 멈추지 않음.
