...
- YouTube가 `web_safari` 클라이언트에 영상 포맷 없이 이미지만 반환하면 예전에는 Mix 156곡을 모두 실패한 뒤에야 폴백했다. 첫 요청부터 `web, web_embedded, web_safari` 포맷을 함께 조회해 사용 가능한 오디오를 즉시 선택하도록 바꿘다.
- Mix·일반 재생목록·좋아요 목록은 영상 주소만 먼저 읽은 뒤 yt-dlp를 항목별로 하나씩 실행한다. `Ctrl+C`로 중단해도 완료된 MP3와 다운로드 기록은 남으며, 다음 실행 때 이미 완료된 영상을 건너뛰고 이어서 받는다.

## 기상 알람 연동 알림 확장 (2026-09-16)

기존에 기상 알람 시각(`_todays_wake_alarm_time`)에 맞춰 전자책 이어읽기만 자동 실행되던 것을, "일본어선생님이 매일 오후 9시에 메시지 보내는거같은데 기상알람 뜰때도 메시지 보내면 좋겠어 이직방도 기상알람과 함께 그날 공고관련되서 업데이트 된게있으면 메시지 보내서 알리고 손자병법방도 기상알람 뜨면 다음구절 라이트모드로 분석한다음 토론 시작하면좋겠어" 요청으로 세 방까지 확장했다. 기존 오후 9시 일본어 복습 트리거(`_check_jp_subtitle_daily_review`)는 그대로 유지되고, 아래는 전부 별도로 추가된 것이다. 셋 다 `_check_wake_alarm_ebook_resume`과 같은 1분 주기 시각 체크 + 날짜 단위 dedup 패턴(`self._last_wake_alarm_*_notified`)을 쓴다.

- **일본어 스터디방** (`_check_wake_alarm_jp_teacher_greeting`): 기상 알람 시각에 "좋은 아침 인사와 함께 오늘의 학습 정보를 소개해주세요" 트리거를 추가로 보낸다.
- **이직 준비방** (`_check_wake_alarm_job_prep_update`): 기상 알람 시각에 `_refresh_job_collector(None)`을 직접 호출한다. 원래도 하루 1번(`JOB_COLLECTOR_REFRESH_SECONDS=24h`) 자동 수집이 돌았지만 그 시각이 앱 시작 시점에 매여 있었다 — 기상 알람 시각으로 앵커를 옮긴 것뿐이며, `_refresh_job_collector` 자체의 경과시간·실행중 가드가 이미 있어 중복 실행되지 않는다. 새 공고가 있을 때만 스터디코치가 알리는 기존 로직(`_run_job_collector_thread` → `_notify_job_prep_room`)을 그대로 재사용한다.
- **손자병법 토론방** (`_check_wake_alarm_sunzi_analysis`): 기상 알람 시각에 다음 구절 라이트 모드 분석을 트리거한다. ShiftAlarm은 소유자 세션 쿠키가 없어 대시보드 버튼(`/api/shift-alarm/sunzi-analysis`, 소유자 인증)을 그대로 못 쓴다 — 같은 큐잉 로직(`_enqueue_sunzi_light_analysis`)을 공유하는 WORKER_TOKEN 인증 전용 엔드포인트 `POST /api/worker/sunzi_light_analysis`를 chatapp server(`app.py`)에 새로 만들어 `_start_sunzi_light_analysis()`가 호출한다. 이미 분석이 대기·진행 중이면 서버가 409를 돌려주는데 정상 상황이라 조용히 무시한다. 라이트 파이프라인이 끝나면 기존 자동화(`/api/worker/announcements`)가 토론까지 이어서 시작하므로 여기서는 분석 요청만 보내면 된다 — 별도의 "토론 시작" 호출은 필요 없다.
...
- **★ 2026-08-07 버그 수정(타이틀 통째로 숨겨짐, 2026-08-31 최종 단순화)**: 메뉴바 공간 부족 재발을 막기 위해 리마인더는 타이틀에서 전부 제외했다. 메뉴바에는 근무·날씨, 저장공간, Codex/Claude 사용률, 필요 시 열 경고만 남는다. 예: `G5-晴 14G 42% 18%`. 전체 리마인더는 Pet 카드와 드롭다운에서 확인한다.
...
| 💇 머리 깎는 날 | 근무표와 무관하게 2026-08-30부터 **40일마다 한 번**. 메뉴에서 개별로 끌 수 있고 해당 날짜의 Notion 체크리스트에도 함께 기록됨 (★ 2026-08-13 추가, ★ 2026-08-30 실제로 머리·수염 깎은 날 기준으로 25일→40일 주기 재조정 및 기준일 갱신) |
...
### 8-0. ★ 웹 EPUB 리더와 진행률 공유 — 장 단위 후퇴 사고 수정 (2026-09-22)

**증상**: 아침 리더로 `Tools of Titans`를 50%대(문장 idx 1144, P.2887)까지 읽었는데, 어느 순간 웹 EPUB 리더(`일본어자막추출/web_reader`)에서 이 책을 열었더니 진행률이 28.2%(idx 546)로 되돌아가 있었고, 그 뒤로 아침 리더를 다시 켜도 계속 28%대에서 시작했다.

**근본 원인**: 아침 리더와 웹 리더는 같은 책의 `<파일명>.reader-progress.json` 사이드카를 공유하는데, 두 리더의 위치 단위가 다르다 — 아침 리더는 **문장 번호**(`sentence_idx`/`sentence_total`), 웹 리더는 **EPUB spine(장) 번호**(`spine_index`/`spine_total`)만 안다. 이 책은 장이 6개뿐인데 분량이 극단적으로 들쭉날쭉해서(3번째 장 하나가 전체의 25~60% 구간을 차지), "장 번호"를 "그 장의 첫 문장"으로 변환하면 실제로 그 장 안에서 한참 더 읽은 위치를 통째로 그 장의 시작으로 되돌려버린다. `ebook_reader.py`가 웹 쪽 위치를 무조건 받아들이도록 돼 있어서 이 손실이 그대로 저장·고착됐다.
- `ebook_reader.py`: 웹에서 온 위치(장 시작으로 변환한 값)가 현재 위치보다 **더 앞설 때만** 받아들이도록 수정 — 두 리더를 오가도 진행은 앞으로만 간다.
- `web_reader/server.py`의 `progress_for()`: 퍼센트를 "(장 번호+1)/전체 장 수"로 근사하던 것도 부정확했다(3/6장 = 66.7%로 보이지만 실제 59%). 아침 리더가 준 `sentence_idx`/`sentence_total`이 있으면 그걸로 정밀 퍼센트를 계산하고, `resume_text`(이미 English 리더가 페이지 로드 시 해당 문단으로 스크롤하는 데 씀)로 정확한 위치까지 보여준다.
- `save_progress()`: 웹에서 책을 "열어보기만" 해도(`display()`가 매번 진행률 PUT을 보냄) 이 정밀 정보가 장 단위 근사치로 깎여나가지 않도록, 같은 장을 보는 동안은 아침 리더의 문장 단위 값을 그대로 지킨다 — 실제로 다른 장으로 넘어갈 때만 "web" 소스로 갈아탄다.
- 손실됐던 위치는 `~/.ebook_reader/sessions/`의 세션 로그(날짜별 페이지 범위 기록)를 역추적해 마지막으로 도달한 P.2887(idx 1144, 59.2%)로 수동 복구했다. 회귀 테스트는 `일본어자막추출/web_reader/test_server.py`에 추가.
...
- 메뉴바 타이틀에서는 리마인더 segment를 완전히 제외한다. 미완료 리마인더 원본 토큰은 Pet 카드에만 넘기고 드롭다운 원문도 그대로 유지한다. 저장공간은 `14G`처럼 표시하고 5GB 이하에서도 경고 이모지 대신 수치 자체를 빨간색으로 유지한다. 리마인더를 줄이는 적응형 축약 단계도 더 이상 필요 없어 제거했다.
- 원문 첫 이모지가 같은 두 `☕` 리마인더는 생성 단계에서 `카페`와 `주간정리`라는 고유 의미 토큰으로 분리한다. Pet 카드에는 각각 `카페에서 공부하기`와 `주간 마지막 루틴 하기`가 정확히 표시되며 드롭다운 원문은 바뀌지 않는다.
...
- **배경 제거 + 말풍선 분리**: 이미지+텍스트를 한 사각형에 감싸던 기존 디자인을 버렸다. 이제 이미지(`IMAGE_RECT`, 배경 없이 그대로)와 말풍선(`BUBBLE_RECT`, 꼬리 달린 둥근 사각형)을 완전히 분리해서 그린다. Pet 전체 크기는 210×152이며 말풍선은 80pt 높이와 줄바꿈 drawing option을 사용해 친절한 2~3줄 안내가 잘리지 않게 한다.
...
- **카드 순환**: shift_alarm이 `(제목, 내용)` 카드 리스트(근무·저장공간·오늘 리마인더·AI 사용량·[열 상태 있을 때만])를 `_update_title()`에서 만들어 `shift_pet.update(cards)`로 넘긴다. 각 카드를 30초 보여준 뒤 말풍선·꼬리·텍스트·×만 10초 숨기고(캐릭터는 유지), 다음 카드를 다시 표시하는 one-shot `NSTimer` 상태 머신이다. 숨김 중에도 캐릭터 드래그와 이미지 클릭은 작동하지만 보이지 않는 × 영역은 반응하지 않는다. `build_pet_cards()`가 `오늘은 G3 근무예요`, `비가 올 수 있어요. 우산 챙겨요!`, `지금 14GB 남았어요`처럼 짧고 친절하게 말한다. 리마인더 카드는 `오늘의 리마인더 인지하셨나요?` 아래에 `상체 운동하기 · 엄마에게 전화하기가 남아 있어요`처럼 미완료 항목만 동작형 문장으로 보여주며, 긴 목록은 앞 2개와 정확한 나머지 개수로 줄인다.
- `ShiftAlarmPet.update(cards)`는 실제 카드 리스트가 달라졌을 때만 첫 카드 visible 상태로 초기화하고 30초 타이머를 다시 시작한다. 급여 갱신 등 동일한 `_update_title()`이 30초마다 호출돼도 현재 카드/숨김 phase를 그대로 두므로 순환이 첫 카드에 고정되지 않는다.
...
## 64. 📖 독서 세션 완료 알림을 캔 문구 → 진짜 AI 요약+저자 논평으로 (★ 2026-08-30 추가)

**사용자 요청**: "이런식으로 말하지 말고 오늘 읽은 내용은 무엇이었는지 간단하게 요약하고 흥미로운점 설명해주는 방식으로 말해줘 그리고 이거에대해서 저자가 직접 답변도해주고."

- `ebook_reader.py`의 `notify_tulpachat_reading_done()`이 예전엔 "📖 오늘 OO 읽었더라!" 캔 문구를 독서지기 명의로 그냥 올렸다 — 이제는 툴파챗 새 엔드포인트 `/api/worker/reading_session_done`으로 시스템 알림 메시지 하나만 남기고, 독서지기·티모시 페리스 둘 다에게 진짜 AI 응답 턴을 배정한다. 각자 오늘 읽은 내용(live_state)을 근거로 실제 요약·논평을 만들어낸다.
- 자세한 내용(엔드포인트 구조, 순차 처리 메커니즘, 실측 결과)은 `툴파챗/chatapp/README.md`의 "독서 세션 완료 알림 — 캔 문구 대신 진짜 AI 요약+저자 논평" 항목 참고.

## 65. 🇯🇵 일본어 스터디방 — 자막 추출 완료 알림 + 하루 하나씩 복습 (★ 2026-09-01 추가)

**사용자 요청**: "일본어 자막추출도 비슷한방식으로 플랜 만들어줘" → "일본어 스터디방으로 제목 붙이고 오늘 추출된 영상에서는 무슨 이야기가 있었는지 무슨 표현이 좋은지 학습카드 위주로 설명하면좋겠고 그리고 만들었던 epub 기반으로해서 하루에 하나씩 복습 했으면 좋겠어."
...
## 67. 💡🎵 일일 루틴 전부 체크되면 거실1 조명 끄기 + 음악 자동 정지 (★ 2026-09-02 추가)

**사용자 요청**: "루틴지기한테 일일체크 전부체크 해달라고 하면 알아서 거실 1 조명 off 하게 해줘 음악도 자동으로 꺼지게 해주고."

- `_fetch_checklist_state_thread()`(주기적 Notion 동기화)가 일일 루틴 상태를 갱신할 때, 직전엔 "전부 체크"가 아니었는데 이번엔 "전부 체크"로 바뀌는 순간만 감지한다 — 메뉴에서 "전부 체크" 버튼을 눌렀든, 루틴지기 채팅에서 승인해서 체크됐든 결국 같은 Notion 데이터를 갱신하므로 출처를 가리지 않고 상태 전이만 본다.
- 감지되면 `_on_daily_routine_all_checked()`가 `set_hue_room_power(HUE_WAKE_ROOM_NAME, False)`로 거실1 조명을 끄고, 기존 `pause_music_if_playing()`(26번 항목 "불끄기 누르면 음악 일시정지"와 같은 함수)으로 재생 중인 음악을 멈춘다 — 둘 다 기존에 있던 함수를 그대로 재사용한 것.

## 68. 🎂 엄마 생신(음력 8/15) 일주일 전 리마인더 + 당일 알람 (★ 2026-09-02 추가)

**사용자 요청**: "8.15일을 음력으로 계산하면 몇월몇일이야? 엄마생일인데 일주일전에 미리 리마인더 해주고 당일에 리마인더 알람 생성해줘." → "어 필요해 매년 알람 해줘."

- 2026년 음력 8월 15일(추석)은 양력 9월 25일(금)로 WebSearch로 복수 출처 교차 확인했다.
...
## 70. 📊 저장공간 상세보기 "눌러도 아무것도 안 나오는" 버그 수정 (★ 2026-09-02 추가)

**사용자 요청**: "저장공간 상세보기 항목 눌러도 아무것도 안나오는데 문제 해결해줘."

- **실측으로 확인한 원인**: 62번 항목(저장공간 상세보기)이 쓰던 순수 파이썬 `os.walk` 기반 `scan_top_level_sizes()`를 실제 홈 폴더에서 직접 돌려봤더니 **5분(300초)을 줘도 끝나지 않았다** — 이 사용자 홈 폴더엔 대용량 저장소·`node_modules`·`.git` 등 파일 수가 아주 많은 폴더가 여럿 있어, "느린" 정도가 아니라 사실상 끝나지 않는 수준이었다(원래 이 방식을 쓴 이유는 `du`가 launchd GUI 에이전트에서 `capture_output`이 조용히 실패하는 문제 때문이었는데, 그 우회가 감당 못 할 만큼 느려진 것).
- `du -d1 -k`(네이티브 바이너리)로 교체하고, `capture_output=True` 대신 임시 파일로 출력을 리다이렉트해 launchd FD 문제를 피했다 — 같은 홈 폴더에서 실측 약 70초로 단축(그래도 즉시는 아님).
- 그래도 70초는 "누르자마자 뭔가 보이길" 기대하는 사용자에게는 길어서, 결과를 `~/.shift_alarm_storage_scan_cache.json`에 캐시해뒀다가 **다음 클릭부터는 캐시를 즉시 보여주고 새 스캔은 뒤에서 조용히 돌려** 다음번엔 최신값이 나오게 했다(첫 실행만 "1~2분 걸릴 수 있다"고 미리 안내하고 기다림).
- 실측: 새 `scan_top_level_sizes()`가 69.9초 만에 정상 결과(Library 35.9GB, Desktop 32.0GB, `.codex` 5.9GB 등)를 반환하는 것과 캐시 저장·재로드 왕복이 정확히 일치하는 것을 직접 확인. 덤으로 이 스캔 결과에서 사용자가 따로 물어본 `art-eye-track`(922MB, 실행 중인 프로세스가 아니라 홈 폴더에 있는 디렉터리였음)도 확인됐다.

## 71. 🏆 추천 경진대회 알림도 이직 준비방에서 논의하게 트리거 추가 (★ 2026-09-02 추가)

**사용자 요청**: "추천경진 알람도 떴는데 이거 관련해서 이직준비방에서 이야기해주면 좋겠어."
...
## 74. 🏆 경진대회 추천 트리거를 전담 페르소나 "경진" 단일 대상으로 변경 (★ 2026-09-03 추가)

**사용자 요청**: "이거 있잖아 그냥 메시지화 하면안되나? 채팅방 누군가 한명 골라서 하던가 경진이라는 페르소나 만들어서 채팅방 초대하고 경진이는 이직시스템에서 경진대회 분야 맡아서 하는걸로하자."

- 71번 항목에서 방 전체(구직지기·커리어코치·스터디코치·업계분석가·기업크롤러)에 데이터를 나열해 던지던 시스템 배너 트리거를, 경진대회 전담 페르소나 "경진" 한 명에게만 짧은 트리거를 보내는 방식으로 바꿨다. `_notify_tulpachat_room()`/`_notify_job_prep_room()`에 `target_persona` 인자를 추가(툴파챗 서버의 `/api/worker/reading_session_done` `target_persona` 확장과 짝) — 실제 소개 문구(주최·제목·마감·링크·후보자 적합도)는 이제 경진이 자기 쪽 live_state를 근거로 자연스러운 채팅 메시지로 직접 만든다(자세한 내용은 `툴파챗/chatapp/README.md`의 같은 제목 항목 참고).

## 75. 💊 Swing→Day 전환 휴무일 20:00 멜라토닌 알림 추가 (★ 2026-09-07 추가)

**사용자 요청**: "swing 에서 day 로 넘어가는 휴일에는 오후 8시에 멜라토닌 먹기 알람 리마인드해줘."

- 기존 GY→Swing 전환 휴무(29·30번 항목)와 같은 패턴이지만, 첫날/둘째날을 나눠 다르게 다뤄달라는 요청은 없었으므로 블록에 속하는 휴무일이면 매일 20:00에 똑같이 알린다.
...
- 실제 값 채우기는 이번 범위에 넣지 않았다 — 스키마와 읽기/우선순위 로직만 만들고, 어떤 리마인더를 어떤 상황에서 다르게 하고 싶은지는 표에서 직접 채워 넣도록 남겨뒀다(카페 리마인더처럼 "휴무 첫날마다 다른 컨텍스트일 수 있는" 항목이 그 예). (★ 79번 참고: 바로 다음 요청으로 이 빈칸도 채웠다.)

## 81. ⏰ 리마인더 시각표 컨텍스트 칼럼에 실제 값 채우기 (★ 2026-09-07 추가)

**사용자 요청**: "이거 시각을 채워주면 좋겠어 빈칸이아니라." (80번에서 스키마만 만들고 빈칸으로 남겨뒀던 Swing/Day/GY/S-D휴/D-G휴/G-S휴 6칼럼에 실제 시각을 채워달라는 요청.)

- 22개 리마인더 각각을 실제 근무시간대(Day 06:00~14:00·Swing 14:00~22:00·GY 22:00~06:00)와 기상시간(Day 02:55·Swing 08:30·GY 16:30·D-G휴 08:00·G-S휴 1일차 18:00/2일차 13:00·S-D휴는 마지막날만 06:00+20:00 멜라토닌)에 안 겹치게 하나씩 검토해서 채웠다. 원칙: (1) 근무 중인 시간대에 걸리면 근무 앞/뒤 여유 시간으로 옮긴다, (2) 오프데이 특정 컨텍스트의 기상시간보다 이른 시각이면 기상 이후로 옮긴다, (3) 충돌이 없으면 빈칸으로 남겨 기본 "시각"을 그대로 쓴다.
...
## 83. 💊 Day/Swing/GY 근무일 멜라토닌 리마인더 신설 (★ 2026-09-09 추가)

**사용자 요청**: "Day,swing, gy 각각에도 멜라토닌 먹는 시각 리마인더 만들어줘." 82번은
기존 전환 휴무(S-D휴/G-S휴) 멜라토닌 알림을 웹 표에 "보여만" 줬을 뿐, 정작 매일
반복되는 일반 근무일(Day/Swing/GY)엔 멜라토닌 알림 자체가 없었다.

- `REMINDERS`에 `melatonin_shift`("💊 멜라토닌 먹을 시각(근무일)") 항목을 추가하고,
  `_get_today_reminder_items`에 `get_shift_for_date(schedule, today) in ("Day","Swing","GY")`
  조건을 걸어 근무일마다(전환 휴무일은 제외) 뜨게 했다 — 다른 22개 리마인더와 동일한
  체크리스트·메뉴·개별 알림(`_check_timed_reminders`) 경로를 그대로 탄다.
- 실제 시각은 기존 S-D휴 마지막날 멜라토닌 규칙(기상 06:00 기준 10시간 전인 20:00)과
  같은 "기상 10시간 전" 원칙으로 계산해 ⏰ 리마인더 시각표 Notion 표의 Swing/Day/GY
  컨텍스트 칼럼에 채웠다: Day 16:55(기상 02:55 기준)·Swing 22:30(기상 08:30 기준)·
  GY 06:30(기상 16:30 기준). S-D휴/D-G휴/G-S휴 칸은 비워 기존 전환 휴무 전용
  멜라토닌 알림(82번 항목들)과 겹치지 않게 했다.
- 365일 근무표로 Day/Swing/GY 날짜마다 뜨고 휴무일엔 전혀 안 뜨는 것, Notion 표
  파싱·컨텍스트별 시각 해석이 올바른 것을 직접 확인했다.

## 84. ⏰ 웹 시각표 "전환 휴무" 기상 알람을 S→D·D→G 개별 행으로 분리 (★ 2026-09-09 추가)

**사용자 요청**: "휴무일 기상알람이 g-s 때밖에 없는데 나머지 휴무일 기상도 시각표에 추가해줘."

- 원인: `build_sleep_schedule()`의 `wake_transition` 행 하나에 S-D휴·D-G휴 시각을
  같이 담아뒀는데, 웹 표는 화면에서 고른 프로필(시각/Swing/Day/GY/S-D휴/D-G휴/G-S휴)
  칼럼 값 하나만 그 행에 보여주는 구조다(`app.js`의 `(item.times||{})[profile]`).
  그래서 G-S휴 프로필로 보면 이 행은 S-D휴·D-G휴 키만 있어 통째로 빈칸으로
  보였다 — 값이 없던 게 아니라 한 행에 두 프로필을 몰아넣어 항상 절반은
  안 보이는 구조였다.
- `wake_transition`을 G-S휴 첫날·둘째날처럼 `wake_swing_to_day`("⏰ 기상 알람
  (S→D 휴무 마지막날)")·`wake_day_to_gy`("⏰ 기상 알람 (D→G 휴무)") 두 행으로
...
## 86. 🥥 코코넛오일 사는 날 리마인더 — 2개월 주기 (★ 2026-09-12 추가)

**사용자 요청**: "오늘 코코넛오일 살건데 주기 두달로 맞춰서 리마인더 추가해".
...
## 90. 📥 Shift Alarm Mac→iPhone 영상 다운로드 (★ 2026-09-14 추가)

- 소유자 전용 웹 대시보드에서 허가된 스트리밍 사이트 또는 `m3u8` HTTPS 주소를 입력하고 확인 창에서 승인하면 Mac의 고정 작업기가 다운로드를 시작한다.
- AI나 페르소나에 Bash 권한을 주지 않는다. 서버는 권한이 `0600`인 요청 파일을 만들고 `stream_download_worker.py`만 실행하며, 작업기는 고정된 `yt-dlp` 옵션과 `ffmpeg` MP4 병합만 사용한다.
- 비HTTPS·인증정보 포함 URL·내부망 IP·동시 중복 작업을 차단하고 파일 크기는 최대 5GB로 제한한다. DRM 또는 로그인 보호를 우회하지 않는다.
- 진행률과 실패 원인을 대시보드가 3초마다 표시한다. 완성 파일은 로컬 스테이징 뒤 기존 `iCloudSync.app`에 복사를 위임해 `iCloud Drive/Shift Alarm Downloads`로 보낸다.
- 완료 알림을 누른 뒤 iPhone 파일 앱에서 영상을 길게 눌러 `나의 iPhone → nPlayer → 루틴`으로 이동한다. iOS 앱 전용 로컬 폴더에는 서버가 직접 쓸 수 없으므로 마지막 이동은 사용자가 수행한다.

## 91. 📲 Mac 비공개 보관함에서 iPhone 직접 다운로드 (★ 2026-09-14 추가)

- iCloud를 기본 전달 경로에서 제외했다. 완성 영상은 일본어 자막 추출의 `av완성작`과 같은 상위 폴더에 있는 `/Users/forrestdpark/Desktop/BlogImage/av4`에 24시간 보관하고, SQLite에는 소유자·안전한 파일명·경로·크기·완료/만료 시각만 저장한다. 영상 바이트 자체를 DB에 넣지는 않는다. 만료 정리는 DB에 등록된 Shift Alarm 다운로드만 삭제하며 기존 `av4` 파일은 건드리지 않는다.
- 소유자 인증을 통과한 기기만 다운로드할 수 있으며 HTTP Range 응답을 지원해 대용량 영상의 재생·이어받기가 가능하다. 만료된 파일은 상태 조회 또는 다음 작업 시작 시 결정론적 코드가 삭제한다.
- 대시보드의 기본 버튼은 `iPhone으로 다운로드`다. `Mac에서 AirDrop 준비`는 Finder에서 파일과 AirDrop 창을 열 뿐 대상 선택과 iPhone 수락은 자동화하지 않는다. `iCloud로 보내기`는 선택형 보조 경로로 유지한다.
- 다운로드 후 iOS 보안 정책상 `나의 iPhone → nPlayer → 루틴`으로의 마지막 이동은 사용자가 파일 앱에서 수행한다.
- `yt-dlp --print`가 암묵적으로 진행률 출력을 숨기는 문제를 막기 위해 `--progress --progress-delta 1`을 명시한다. 작업기는 실제 다운로드 바이트·예상 전체 바이트·퍼센트를 상태 파일에 기록하고, 구형 실행 작업도 서버가 스테이징 파일 크기를 읽어 최소한 실제 내려받은 용량을 표시한다.

## 92. ⛽☕ 차 기름 넣기·커피 구매 리마인더 — 각 1개월 주기 (★ 2026-09-17 추가)

**사용자 요청**: "오늘 차 기름넣기 주기한달로 해서리마인더 추가하고 커피구매하기도 한달주기로 리마인더 추가해줘".

- `REMINDERS`에 `car_refuel`·`coffee_purchase` 두 항목을 추가하고, 새 헬퍼 `_is_car_refuel_day(d)`·`_is_coffee_purchase_day(d)`로 판정한다. 기준일(둘 다 2026-09-17, 요청한 날)부터 "달(month)" 단위로 주기를 세는 방식이 `engine_oil_change`(5개월)·`coconut_oil`(2개월)과 완전히 같아 그 구현을 그대로 재사용했다 — `(year*12+month) 차이 % 1개월 == 0`이고 `day`가 기준일(17일)과 같은지 확인한다. 기준일이 17일이라 모든 달에 존재하므로 말일 보정은 불필요.
- `_get_today_reminder_items()`의 다른 월 단위 조건들과 같은 자리에 추가해서 메뉴 표시·음성 낭독·토글·Notion 일일 체크리스트 동기화 등 기존 인프라를 그대로 탄다.
- 실행 시각은 차 기름 12:30, 커피 구매 12:45로 지정(엔진오일 12:00·코코넛오일 12:15 바로 뒤 순서로 이어붙임).

## 93. 웹앱에서 리마인더 추가·수정·삭제와 반복 주기 관리 (★ 2026-09-20 추가)

- Shift Alarm 대시보드의 리마인더 관리에서 새 항목을 만들고 이름·시각·사용 여부를 수정하거나 영구 삭제할 수 있다.
- 반복 주기는 매일, N일, N주, N개월 단위와 기준일을 지원한다. 기존 휴무·교대근무 기반 리마인더는 `기존 규칙`으로 유지되며, 사용자가 일반 반복 주기를 선택한 항목만 새 규칙으로 대체된다.
- 웹 서버는 `~/.shift_alarm_reminders.json`에 검증된 설정만 원자적으로 저장한다. 실제 날짜 판정과 알람 실행은 계속 `shift_alarm.py`의 결정론적 코드가 담당한다.
- 오늘 목록의 `삭제`는 오늘 한 번만 숨기는 기존 기능이고, 관리 표의 `영구 삭제`는 반복 정의 자체를 제거하므로 두 동작을 분리했다.

## 94. 🔇📝 CPU 과부하 자동 종료 — 알람 무음화 + 로그 기록 (★ 2026-09-23 추가)

**배경**: CPU 과부하로 30분 이상 이어져 자동 종료된 프로세스가 있었는데, 알림 배너가 지나간 뒤에는 무엇이 종료됐는지 확인할 방법이 없었다. 알림 센터 DB(`~/Library/Group Containers/group.com.apple.usernoted`, `com.apple.notificationcenter/db2`)도 확인해봤지만 SIP로 막혀 있고 실제 DB 파일도 없어 복구 불가. 사용자 요청: "일단 로그 남기도록 수정하고 이거 알람소리 너무크니까 시피유 어쩌구 알람은 소리 안나도록해".
