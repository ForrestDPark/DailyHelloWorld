...
T(ax, '한신 배수진 (1만)', 7.6, MAP_Y0 + 1.0, sz=36, c='#7a0000', bg='#fbeaea')

# ── 한군 본대: 정형관에서 나와 동쪽으로 진격 (깃발·북 과시, 미끼) ──
n2 = 40
...
arr(ax, 3.6, MAP_Y0 + 5.5, 7.5, MAP_Y0 + 5.4, '#ee2222', rad=0.05)
...
# 조군 돌격 (배수진 향해 전군 투입)
arr(ax, 14.5, MAP_Y0 + 5.5, 9.4, MAP_Y0 + 5.4, '#2255dd', rad=-0.08, hw=4.5, tw=2.6)
...
# ── 지휘관 표식 ──
pe_em(ax, '⚔️', 8.0, MAP_Y0 + 6.6, zoom=0.24, z=17)
...
draw_analysis_box(ax, renderer, LX, '패군', '진여 (성안군)', 패군_rows, '#7a2020')
...
draw_analysis_box(ax, renderer, LX + 2 * (BW + GAP), '승군', '한신', 승군_rows, '#1a3a7a')
