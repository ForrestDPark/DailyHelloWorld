...
- 각 큰 장면 시작에는 대사 페이지와 분리된 `장면 학습 카드`를 먼저 넣는다. 순서는 **주요 단어와 뜻 / 핵심 일본어 표현 / 문법·어미·뉘앙스 / 쉐도잉 추천 문장**이며, 핵심 표현은 고정 3개가 아니라 장면에서 실제로 나온 표현 중 약 10개(6~10개)를 선별한다. 내용이 많으면 카드를 2쪽 이상으로 자동 분할한 뒤 다음 쪽에서 장면 첫 대사를 시작하므로 카드와 대사가 겹치면 안 된다.
