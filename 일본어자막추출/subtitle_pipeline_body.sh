...
# ── 학습카드 누락 회차 자동 복구 (함수 정의만 — 호출은 메인 루프 뒤) ─────
# ★ 2026-09-24: "학습카드없는거 재시도하는건 좋은데 일단 먼저시킨 자막생성
# 다하고 끝나고난다음 하면좋겠어" 요청 — 예전엔 새 영상 처리 전에 이 복구를
# 먼저 돌려서(AI 요약·시나리오·이미지 생성이 길다) 사용자가 방금 시킨 자막
# 생성이 한참 뒤로 밀렸다. 정의는 그대로 두고 호출만 아래 메인 처리 루프가
# 전부 끝난 뒤로 옮겼다. (함수 안 본문은 임베디드 파이썬 때문에 들여쓰지 않음)
