...
OWN_OUTPUT_MARKER = "_고음영상"


def collect_videos(path):
    """path(폴더/파일)에서 처리할 원본 영상을 모은다.
    ★ 2026-07-24: 폴더 안에 이 스크립트가 예전에 만든 결과물(파일명에
    '_고음영상'이 들어간 것 — 고음영상 자체와 _bgm 버전 둘 다)이 원본과 같이
    있을 수 있어서, 재추출 시 그것들을 다시 원본인 척 처리하지 않도록 제외한다."""
    if os.path.isdir(path):
        files = []
        for ext in VIDEO_EXTS:
            files.extend(glob.glob(os.path.join(path, f"*{ext}")))
        files = [f for f in files if OWN_OUTPUT_MARKER not in os.path.basename(f)]
        return sorted(files)
...
def fmt_duration_compact(seconds):
    """제목/파일명용 짧은 길이 표기 — 콜론은 macOS 파일명에서 문제될 수 있어 h/m/s로 표기."""
...
def process_video(video_path, args):
    ...
    # ★ 2026-07-24: 평균-최고 음역 차이가 작으면(다이나믹 레인지가 좁으면) 35%
    # 기준은 너무 느슨해서 거의 절반을 다 뽑아버려 "하이라이트"의 의미가 없어진다
    # — 이런 영상은 15%로 더 빡빡하게 걸러서 진짜 튀는 구간만 남긴다.
    # --top-percent를 사용자가 직접 지정했을 땐(비교 테스트 등) 자동조정하지 않는다.
    ...
    # ★ 2026-07-24: 파일명(제목)에 평균/최고 주파수 + 최종 길이를 넣어서,
    # 여러 --top-percent로 뽑아본 결과들을 파일명만 보고 바로 비교할 수 있게 함.
    stats_tag = f"avg{avg_pitch:.0f}Hz_peak{peak_pitch:.0f}Hz_{fmt_duration_compact(total_dur)}"
    ...
    bgm_out = os.path.join(work_dir, f"{base}{OWN_OUTPUT_MARKER}_{stats_tag}_bgm.mp4")
...
def main():
    ...
    ap.add_argument("--pad", type=float, default=0.8,
                     help="추출 구간 앞뒤 여유 (초, 기본 0.8 — 2026-07-24, 0.3은 너무 촉박하다는 피드백으로 조정)")
